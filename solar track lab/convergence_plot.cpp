...
int main(){
    ...
    std::for_each(file_names.begin(), file_names.end(), [&graphs](std::string s) { 
     ...
     pallette.push_back({255,100,100});	
     pallette.push_back({100,255,100});	
     

   //  std::for_each(graphs.begin(), graphs.end(), [](Graph g){ print_graph(g);} );
     // assign colors
     unsigned int count = 0;
     std::for_each(graphs.begin(), graphs.end(), [pallette, &count](Graph& g){ 
         g.rgb = pallette.at(count); 
         count++; 
         if (count>=pallette.size()) { count = 0;};
      });

...
struct Net{
	...
	void init(unsigned int n_inputs, unsigned int n_hidden,unsigned int n_outputs);
    void print();
...
void Net::init( unsigned int n_inputs, unsigned int n_hidden,unsigned int n_outputs){
...
void Net::backprop(const train_point_t& tp){
	...
	for(Neuron& nhl:hidden_layer){
		...
		for ( unsigned int i = 0 ; i < nhl.weights.size(); i++){
...
void Net::train(train_set_t ts){
	...
	while ((dataset_error>0.0001)&&(epoch<n_epoch)){
		for (unsigned int i = 0; i < ts.train_points.size() ; i = i + 1){
			if (n_batch == 1){
...
int main(int argc, char* argv[]){
	...
	std::string train_set_file_name = "C:/Users/xboxs/Documents/Uni/ENGR110/ML project/mnist_train.csv";

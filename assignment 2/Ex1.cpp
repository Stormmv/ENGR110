...
void gradient_search(Neuron& neuron,const train_set_t& train_set){
    ...
    while ((current_tot_err>0.01)&&(n_step<150)){
		...
		for (unsigned int i = 0 ; i < train_set.train_points.size(); i++){
            ...
            double delta = (sigmoid(neuron.z)*(1-sigmoid(neuron.z))*(neuron.y-train_set.train_points[i].target));
            db = delta;
            dw0 = delta*train_set.train_points[i].inputs[0];
            dw1 = delta*train_set.train_points[i].inputs[1];
